...
CREATE INDEX playlist_department_index
  ON playlist (departmentIDfk);
CREATE TABLE screen
(
  ID             INT(10) UNSIGNED PRIMARY KEY NOT NULL AUTO_INCREMENT,
...
CREATE INDEX screen_department_ID_fk
  ON screen (departmentIDfk);
CREATE INDEX screen_playlist_index
  ON screen (playlistIDfk);
CREATE TABLE slide
(
  ID              INT(10) UNSIGNED PRIMARY KEY        NOT NULL AUTO_INCREMENT,
...
CREATE INDEX slide_playlist_ID_fk
  ON slide (playlistID);
CREATE TABLE user
(
  ID             INT(10) UNSIGNED PRIMARY KEY     NOT NULL AUTO_INCREMENT,
...
CREATE UNIQUE INDEX sNumber
  ON user (sNumber);
CREATE INDEX user_department_fk_idx
  ON user (departmentIDfk);